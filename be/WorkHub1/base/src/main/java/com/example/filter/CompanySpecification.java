...
/**
 * Dynamic Query Builder for Company
 */
public class CompanySpecification {
    ...
    public static QueryResult buildQuery(FilterExpression filter, String keyword) {
        ...
        if (keyword != null && !keyword.trim().isEmpty()) {
            conditions.add("(name LIKE :kw_name OR description LIKE :kw_desc OR address LIKE :kw_addr)");
            params.and("kw_name", "%" + keyword.trim() + "%");
            params.and("kw_desc", "%" + keyword.trim() + "%");
            params.and("kw_addr", "%" + keyword.trim() + "%");
        }
    ...
    private static void buildConditions(FilterExpression filter, List<String> conditions, Parameters params) {
        ...
        for (FilterExpression group : filter.expressions) {
            ...
            if (!groupConditions.isEmpty()) {
                ...
                // Merge group params into main params
                mergeParams(params, groupParams);
            }
        }
    }

    private static void mergeParams(Parameters target, Parameters source) {
        // Access the internal map using reflection or recreate the query
        // For simplicity, we'll use unique parameter names
    }

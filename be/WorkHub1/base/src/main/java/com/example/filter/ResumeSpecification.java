...
/**
 * Dynamic Query Builder for Resume
 */
public class ResumeSpecification {
    ...
    public static QueryResult buildQuery(FilterExpression filter, String keyword) {
        ...
        if (keyword != null && !keyword.trim().isEmpty()) {
            conditions.add("(email LIKE :kw_email OR url LIKE :kw_url)");
            params.and("kw_email", "%" + keyword.trim() + "%");
            params.and("kw_url", "%" + keyword.trim() + "%");
        }
    ...
    private static String mapFieldName(String field) {
        ...
        if (f.equals("url")) return "url";
        if (f.equals("status")) return "status";
        if (f.equals("userid") || f.equals("user_id")) return "user.id";
        if (f.equals("jobid") || f.equals("job_id")) return "job.id";
        if (f.equals("companyid") || f.equals("company_id")) return "job.company.id";
    ...
    private static String getValidSortField(String sortBy) {
        ...
        if (s.equals("status")) return "status";

        ...
        jobApiService.updateJob(id, request).enqueue(new Callback<ApiResponse<JobResponse>>() {
            ...
            public void onResponse(Call<ApiResponse<JobResponse>> call, Response<ApiResponse<JobResponse>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    android.util.Log.e("JobRepository", "updateJob failed HTTP " + response.code());
                    try {
                        if (response.errorBody() != null) {
                            android.util.Log.e("JobRepository", "updateJob error body: " + response.errorBody().string());
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
            ...
            @Override
            public void onFailure(Call<ApiResponse<JobResponse>> call, Throwable t) {
                android.util.Log.e("JobRepository", "updateJob network error: " + t.getMessage());
                jobData.setValue(null);
            }
        });
        return jobData;
        ...
        jobApiService.deleteJob(id).enqueue(new Callback<ApiResponse<Object>>() {
            ...
            public void onResponse(Call<ApiResponse<Object>> call, Response<ApiResponse<Object>> response) {
                if (response.isSuccessful()) {
                    // DELETE thành công dù body có null hay không
                    ApiResponse<Object> result = response.body();
                    if (result == null) {
                        // Tạo response giả đánh dấu thành công
                        result = new com.example.timviecapp.models.common.ApiResponse<>();
                        result.setSuccess(true);
                    }
                    jobData.setValue(result);
                } else {
                    android.util.Log.e("JobRepository", "deleteJob failed: " + response.code());
                    jobData.setValue(null);
                }
            }
            ...
            public void onFailure(Call<ApiResponse<Object>> call, Throwable t) {
                android.util.Log.e("JobRepository", "deleteJob error: " + t.getMessage());
                jobData.setValue(null);
            }
        });
        return jobData;
